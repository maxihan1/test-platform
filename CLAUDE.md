...
- **DB 테스트 fixture**: 접두사는 **갈래마다 고유하게**, 정리 구문(`DELETE ... LIKE`)은 **자기 것에만** 맞게 쓴다.
  ...
  **`XWA`·`XWAR`·`XWS`·`XWU`·`XRC`·`XWV`·`XWO` 는 `authoring_request` 를 `LIKE` 가 아니라 자기 `service_id` 로만 지운다** —
...
**예외 하나 — 역방향**(`docs/spec/도메인/작성.md` §3.6 「★ 역방향」, 2026-09-25)의 **미확정 케이스**만 화면에서 본 값을 기대값으로 쓴다.

...
### 1.3 공용 파일을 건드리지 않는다
`packages/kit/`, `docker-compose.yml`, `db/migrations/`는 **Phase 0에서 확정**됐다.
어느 워크스트림도 수정하지 않는다. 필요하면 1.2 절차를 따른다.
...
**같은 유형이 이미 한 번 적혀 있으면** 항목을 또 추가하지 말고,
LEARNINGS.md의 승격 규칙에 따라 SPEC / CLAUDE.md / spec-review 중
맞는 곳에 규칙으로 올릴 것을 제안한다. 제안만 하고 직접 고치지 않는다.

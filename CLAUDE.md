...
**맥 작성 에이전트의 자식 세션은 `/tpx` 가 아니라 `tpx-author` 를 탄다** (2026-09-23).
git·PR 은 맥 스크립트가 하고 자식은 케이스만 만든다. 정본은 `docs/spec/도메인/작성.md` §3.6 「맥 경로는 전용 체인이다」.
...
**예외 (2026-09-23 게이트 1 에서 사용자 승인)** — `tests/<폴더>/*.spec.ts`·`docs/cases/*.md` 만 바뀐 push 는
검사 기록을 요구하지 않는다. 판정은 `.claude/scripts/cases-only.mjs`, 정본은 `docs/HOOKS.md` 「테스트만 바뀐 PR·push 는 가벼운 길」.

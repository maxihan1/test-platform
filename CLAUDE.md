...
1. `docs/SPEC.md`를 읽는다 — 이것은 **색인**이다. 스펙 본문은 `docs/spec/` 아래
   공통 7장·도메인 5장으로 나뉘어 있다. 색인의 "무엇을 하려는가 → 무엇을 읽는가"에서
   이번 작업이 읽을 장을 찾아 **그 장만** 읽는다. 전문을 통째로 읽지 않는다
2. `docs/LEARNINGS.md`를 읽는다 — 이전 세션들이 겪은 것이 여기 있다.
   읽지 않으면 같은 삽질을 처음부터 반복한다
3. `docs/WORKSTREAMS.md`에서 **이번 세션이 맡은 워크스트림**을 확인한다
4. `docs/WORKFLOW.md`에서 지금이 어느 단계인지 확인한다
5. `docs/progress/<WS-ID>.md`가 있으면 읽어 이전 진행 상황을 파악한다
6. 작업 계획을 3~7줄로 요약해 보고한 뒤 **승인을 기다린다**

승인 전에 파일을 만들거나 고치지 않는다.

---

## 1. 절대 규칙 (어기면 병렬 작업이 깨진다)

### 1.1 담당 폴더 밖을 수정하지 않는다
`docs/WORKSTREAMS.md`의 소유 경로 표에 적힌 폴더만 건드린다.
다른 워크스트림의 파일이 잘못돼 보여도 **고치지 말고 보고한다.**
...
`.claude/scripts/guard.mjs`의 `LOCKED`·`OWNED`, 검사기, 코드 중 어디에도 걸려 있지 않으면
...
- 검사는 `spec-review` A1~A3이 사후에 한다 — `git diff main...HEAD` 와 SPEC을 한 줄씩 대조한다
...
마지막 줄이 가장 늦게 드러난다. 소유 밖이면 고치지 말고 **그 갈래 킥오프에 항목으로 넘긴다**(§1.1).
...
- **DB 테스트 fixture**: 접두사는 **갈래마다 고유하게**, 정리 구문(`DELETE ... LIKE`)은 **자기 것에만** 맞게 쓴다.
  ...
  WS-A `ZZA` · WS-B `XBS`·`XBR`·`XBQ`·`XBX`. **WS-D·WS-F는 `ZZ`로 시작하는 것을 쓰지 않는다.**
...
- `git push --force`, 브랜치 삭제, 마이그레이션 되돌리기

...
export const spec = defineCase({
  ...
  expected: z.object({
    shown: z.string().describe('Active 를 골랐을 때 목록에 보일 할 일을 쉼표로 이은 것').default('우유 사기'),
  }),
});
...
test(spec, async ({ page, params, expected }) => {
  ...
  await test.step('보기에서 「Active」를 고른다', async () => {
    ...
    await verify('목록에 「우유 사기」만 보인다', (await page.getByTestId('todo-title').allInnerTexts()).join(', '), expected.shown);
  });
});

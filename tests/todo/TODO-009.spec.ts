...
export const spec = defineCase({
  ...
  expected: z.object({
    left: z.string().describe('완료분을 지운 뒤 목록에 남을 할 일을 쉼표로 이은 것').default('우유 사기'),
  }),
});
...
test(spec, async ({ page, params, expected }) => {
  ...
  await test.step('완료분 지우기 버튼을 누른다', async () => {
    await page.getByRole('button', { name: 'Clear completed' }).click();
    await page.getByRole('button', { name: 'Clear completed' }).waitFor({ state: 'detached' });
    await verify('목록에 「우유 사기」만 남아 있다', (await page.getByTestId('todo-title').allInnerTexts()).join(', '), expected.left);
  });
});

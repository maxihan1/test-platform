...
**첫 push 가 자동 권한 검사에 「Data Exfiltration」으로 거부되면 우회하지 않는다** (2026-09-23 · 2026-09-26 두 번).
사용자에게 두 길을 준다 — ① 프롬프트 맨 앞에 `!` 를 붙여 `! git -C <작업방> push -u origin <브랜치>` ② 설정에 git push 허용.
**보냈다는 말을 믿지 말고 `git ls-remote --heads origin <브랜치>` 로 올라갔는지 본다** — 2026-09-26 에 `!` 줄이 명령이 아니라 메시지로만 가서 원격이 비어 있었다.
파일은 PR 이 열린 뒤에 쓴다는 순서는 그대로다.

---
name: tp-review
description: /tp 체인 6단계 — 구현 결과를 독립 렌즈로 검사하고 게이트 2 요약을 만든다. 전 등급 호출. 직접 부르지 않는다.
---
...
**게이트 2 를 막는 기계 장치는 없다.** CI 자동 실행이 없으므로 차단자는 사용자 한 사람뿐이다.
「CI 가 알아서 막겠지」로 넘기지 않는다.

## 선행 읽기

**없음. 재로드 금지.** `/tp` 와 `/tp-impl` 이 이미 실었다. 그 사본을 렌즈 프롬프트에 붙인다.
...
`/qa-only` 처럼 gstack 렌즈를 부를 때는 `/tp-plan-review` 와 같은 규약을 쓴다.
...
**요약을 다 쓴 것이 질문을 낸 것이 아니다.** `/tp` 가 `AskUserQuestion` 으로 받는다.
**병합은 이 스킬이 하지 않는다** — 컨트롤러가 `/tp-merge` 를 부른다.
...
- **지적이 3회 반복** — 작업 자체에 근본 문제. `/tp-spec` 되돌림을 제안한다

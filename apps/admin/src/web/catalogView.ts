...
export function 빈이유(형편: {
  ...
  if (형편.scannedAt === null) {
    return {
      무엇: '아직 케이스를 불러오지 않았습니다',
      왜: '테스트 코드를 훑어 실행할 수 있는 케이스 목록을 만듭니다. 코드가 진실의 원천이라 목록은 그때마다 새로 만들어집니다',
      버튼: '케이스 불러오기',
    };
  }
  ...
  if (형편.건조건) {
    return {
      무엇: 형편.친글자 === '' ? '조건에 맞는 케이스가 없습니다' : `'${형편.친글자}'에 맞는 케이스가 없습니다`,
      왜: '거른 조건을 지우면 전체가 다시 보입니다',
      버튼: '조건 초기화',
    };
  }

  return {
    무엇: '불러왔지만 케이스가 하나도 없습니다',
    // 「하나도 없다」만 말하고 왜인지 안 말하면 사람이 할 수 있는 일이 없다 (SPEC §8.1).
    // 사유(중복 tcId·규칙 위반)는 화면 위 스캔 결과 줄이 이미 보여준다
    왜: '테스트 폴더가 비었거나 규칙에 걸려 읽지 못한 파일이 있습니다. 사유는 위 스캔 결과에 있습니다',
    버튼: '다시 스캔',
  };
}

...
describe('증적 문서 버튼 (SPEC §8.4)', () => {
  ...
  it('엑셀을 만드는 중이면 엑셀만 안 눌린다. 새로고침할 때마다 또 눌리면 문서 행이 쌓인다', () => {
    const 줄 = 증적버튼들('FINISHED', [증적('XLSX', 'PENDING')], 'operator', 'ko');
  ...
  it('PDF 가 실패했으면 PDF 에만 사유와 다시 만들기가 붙는다', () => {
    const 줄 = 증적버튼들('FINISHED', [증적('PDF', 'FAILED')], 'operator', 'ko');
  ...
  it('이미 만든 형식만 다시 만들기가 된다. 같은 실행의 증적은 언제 뽑아도 같다', () => {
    const 줄 = 증적버튼들('FINISHED', [증적('PDF', 'READY')], 'operator', 'ko');
    expect(버튼(줄, 'PDF')).toMatchObject({ 누를수있나: true, 글: 'PDF 다시 만들기' });
    expect(버튼(줄, 'HTML')?.글).toBe('HTML 만들기');
  });

  it('중단된 실행도 만들 수 있다. 여기까지 돌았다가 증적으로 성립한다', () => {
    const 줄 = 증적버튼들('ABORTED', [], 'operator', 'ko');
    expect(줄?.버튼들.every((it) => it.누를수있나)).toBe(true);
  ...
  it('실패한 뒤 다시 만들어 성공하면 옛 사유가 안 남는다', () => {
    ...
    const 줄 = 증적버튼들('FINISHED', [증적('PDF', 'FAILED'), 증적('PDF', 'READY')], 'operator', 'ko');
    expect(버튼(줄, 'PDF')?.사유).toBe(null);
    expect(버튼(줄, 'PDF')?.글).toBe('PDF 다시 만들기');
  });

  it('성공한 뒤 다시 만들다 실패하면 그 사유를 보여준다', () => {
    const 줄 = 증적버튼들('FINISHED', [증적('PDF', 'READY'), 증적('PDF', 'FAILED')], 'operator', 'ko');
    expect(버튼(줄, 'PDF')?.사유).toBe('스크린샷을 찾지 못했습니다');
  });

  it('보기만 등급에게는 만들기 버튼이 아예 없다 (SPEC §3.5)', () => {
    expect(증적버튼들('FINISHED', [], 'viewer', 'ko')).toBe(null);
  });
});

describe('만든 문서를 어떻게 받나 (SPEC §8.4)', () => {
  it('PDF 는 새 창에서 연다. 제출물이고 눌러서 바로 보인다', () => {
    expect(받는법('PDF', 'ko')).toMatchObject({ 글: '열기 ↗', 새창: true });
  });

  it('HTML 도 새 창이다. Ctrl+F 가 된다', () => {
    expect(받는법('HTML', 'ko')).toMatchObject({ 글: '열기 ↗', 새창: true });
  });

  it('엑셀은 브라우저가 못 연다. 받기로 내려받는다', () => {
    expect(받는법('XLSX', 'ko')).toMatchObject({ 글: '받기', 새창: false });
  });

  it('대소문자를 가리지 않는다. 서버가 소문자로 저장할 수도 있다', () => {
    expect(받는법('pdf', 'ko').새창).toBe(true);
    expect(받는법('xlsx', 'ko').새창).toBe(false);
  });

  it('모르는 형식은 받기로 둔다. 열린다고 해 놓고 파일이 떨어지면 사람이 찾는다', () => {
    expect(받는법('무엇인가', 'ko')).toMatchObject({ 글: '받기', 새창: false });
  });

  it('만든 문서 목록도 버튼과 같은 이름을 쓴다. 한 화면에 같은 물건이 두 이름이면 안 된다', () => {
    expect(받는법('XLSX', 'ko').라벨).toBe('엑셀');
  });

  it('모르는 형식은 서버가 준 글자를 그대로 쓴다. 이름이 사라지면 어느 줄이 무엇인지 못 읽는다', () => {
    expect(받는법('무엇인가', 'ko').라벨).toBe('무엇인가');
  });
});

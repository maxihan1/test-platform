...
describe('자리 넷', () => {
  it('설정은 운영 등급에게만 뜬다. 흐리게가 아니라 아예 없다', () => {
    expect(자리목록('admin', 'ko').map((자리) => 자리.이름)).toEqual(['테스트 케이스', '실행 기록', '그래프', '설정']);
  });

  it('실행까지 등급에게 설정 자리는 없다', () => {
    expect(자리목록('operator', 'ko').map((자리) => 자리.이름)).toEqual(['테스트 케이스', '실행 기록', '그래프']);
  });

  it('보기만 등급에게도 설정 자리는 없다', () => {
    expect(자리목록('viewer', 'ko').map((자리) => 자리.이름)).toEqual(['테스트 케이스', '실행 기록', '그래프']);
  });

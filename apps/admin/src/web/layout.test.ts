...
describe('자리 넷', () => {
  it('설정은 운영 등급에게만 뜬다. 흐리게가 아니라 아예 없다', () => {
    expect(자리목록('admin').map((자리) => 자리.이름)).toEqual(['테스트케이스 목록', '실행 기록', '그래프', '설정']);
  });

  it('실행까지 등급에게 설정 자리는 없다', () => {
    expect(자리목록('operator').map((자리) => 자리.이름)).toEqual(['테스트케이스 목록', '실행 기록', '그래프']);
  });

  it('보기만 등급에게도 설정 자리는 없다', () => {
    expect(자리목록('viewer').map((자리) => 자리.이름)).toEqual(['테스트케이스 목록', '실행 기록', '그래프']);
  });
...
describe('배정받은 서비스가 없을 때', () => {
  ...
  it('설정 화면은 덮지 않는다. 가라고 한 곳이 막히면 아무것도 못 한다', () => {
    ...
    expect(빈띠사유(사람('admin', []), '#/settings')).toBe(null);
  });

  it('설정을 못 여는 등급에게는 설정 자리에서도 사유를 보여준다', () => {
    // 그 사람에게는 설정이 길이 아니다. 비워 두면 왜 빈지 알 수 없다
    expect(빈띠사유(사람('viewer', []), '#/settings')?.다음).toContain('요청');
  });

  it('자리를 안 주면 지금까지처럼 군다', () => {
    expect(빈띠사유(사람('admin', []))?.무엇).toBe('아직 배정받은 서비스가 없습니다');
  });
});

...
import type { ItemStatus, RunItemSummary } from './api.js';
...
describe('RunProgressModal (SPEC §8.9)', () => {
  ...
  it('도는 중에는 닫는 버튼 하나만 두고 못 누르는 버튼을 놓지 않는다', () => {
    const onClose = vi.fn();
    render(<RunProgressModal data={도는중실행} onClose={onClose} />);
  ...
  it('끝나면 같은 컴포넌트가 완료 내용을 그리고 실패한 케이스 이름이 보인다', () => {
    render(<RunProgressModal data={끝난실행} onClose={() => {}} />);
  ...
  it('사람이 멈춘 실행은 끝났다고 적지 않는다', () => {
    render(<RunProgressModal data={{ ...끝난실행, status: 'ABORTED' }} onClose={() => {}} />);
  ...
  it('판정을 색만으로 전달하지 않는다 — 숫자 칸마다 글자가 있다', () => {
    for (const data of [도는중실행, 끝난실행]) {
      const { unmount } = render(<RunProgressModal data={data} onClose={() => {}} />);

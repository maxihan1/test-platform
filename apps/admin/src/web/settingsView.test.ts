...
describe('서비스 색의 명암비 (DESIGN.md)', () => {
  it('흰 면 기준으로 잰다. 네모와 경계선이 자리 넷 줄 위에 놓인다', () => {
  ...
  it('DB 의 DEMO 서비스 색이 새 기준은 통과한다', () => {
    ...
    expect(명암비('#888888')).toBeGreaterThan(3);
    expect(색사유('#888888')).toBe(null);
  });
  ...
  it('옛 기본색도 새 기준은 통과한다. 기준이 같은 방향으로 느슨해졌을 뿐이다', () => {
    ...
    expect(색사유('#5B7FDE')).toBe(null);
  });

  it('너무 밝은 색은 여전히 막는다. 흰 면 위에서 안 보인다', () => {
    expect(색사유('#6FBFA6')).not.toBe(null);
  });

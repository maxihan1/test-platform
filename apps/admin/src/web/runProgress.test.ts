...
const 네칸합 = (막대: 진행['막대']) => 막대.통과 + 막대.실패 + 막대.미실행 + 막대.미확정 + 막대.남은것;
...
describe('진행상황', () => {
  ...
  it('막대 네 칸은 counts 를 그대로 옮긴다', () => {
    expect(진행상황(중단됨, []).막대).toEqual({ 통과: 5, 실패: 1, 미실행: 4, 미확정: 0, 남은것: 0 });
  });

  it('끝난 미확정 항목은 따로 한 칸이고 칸의 합이 여전히 counts.total 이다', () => {
    const 섞임 = 실행([...끝난여섯, ...안끝난넷], {
      total: 10,
      pass: 3,
      fail: 1,
      na: 0,
      running: 4,
      unconfirmed: { total: 3, pass: 1, fail: 1, na: 0 },
    });
    const { 막대 } = 진행상황(섞임, []);
    expect(막대).toEqual({ 통과: 3, 실패: 1, 미실행: 0, 미확정: 2, 남은것: 4 });
    expect(네칸합(막대)).toBe(섞임.counts.total);
  });

...
// 케이스 상세 펼침 검사 (SPEC §8.1, 2026-09-21 ②).
//
...
describe('케이스 상세 펼침', () => {
  it('접혀 있으면 아무것도 안 그리고 통로도 안 부른다', () => {
    ...
    const { container } = render(<CaseDetail row={케이스} 폈나={false} 마지막={undefined} />);
  ...
  it('펼치면 손에 이미 있는 것부터 그린다 — 부르기를 기다리지 않는다', () => {
    ...
    render(<CaseDetail row={케이스} 폈나={true} 마지막={undefined} />);
  ...
  it('펼치면 이력을 한 번 부르고, 접었다 다시 펴도 또 부르지 않는다', async () => {
    ...
    const { rerender } = render(<CaseDetail row={케이스} 폈나={true} 마지막={undefined} />);
    await waitFor(() => expect(이력스파이).toHaveBeenCalledTimes(1));

    rerender(<CaseDetail row={케이스} 폈나={false} 마지막={undefined} />);
    rerender(<CaseDetail row={케이스} 폈나={true} 마지막={undefined} />);

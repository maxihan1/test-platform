...
function 작성줄({ 것, 지금 }: { 것: AuthoringRow; 지금: number }) {
  ...
  const 언어 = use언어();
  return (
    <a className="row" href={`#/authoring/${것.id}`}>
      <span className="cell">{종류라벨(것.kind, 언어)}</span>
      <span className="cell">{보임라벨(줄보임(것, 지금), 언어)}</span>
      {/* 단계가 아직 없으면 빈 칸이 아니라 「기록 없음」이다 — 모르는 것을 아는 척하지 않는다 */}
      <span className="cell">{것.stage ?? t('기록 없음')}</span>
      <span className="cell">{것.requestedByName}</span>
    </a>
  );
}
      ...
      <div className="screen list-screen">
        <div className="rows-scroll">
          {줄들.data.items.length === 0 ? (
            ...
            줄들.data.items.map((것) => <작성줄 key={것.id} 것={것} 지금={지금} />)
          )}
        </div>

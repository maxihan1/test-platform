...
describe.skipIf(연결 === undefined)('실행 저장', () => {
  ...
  it('platforms 길이만큼 run_item을 만든다', async () => {
    const run = await createRun({ title: 'XBS 두 환경', triggeredBy: 'tester', items: [{ ...항목, platforms: [...항목.platforms] }] });
  ...
  it('생성 직후에는 판정이 NA이고 끝난 시각이 비어 있다', async () => {
    const run = await createRun({ title: 'XBS 생성 직후', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
  ...
  it('실행 묶음은 RUNNING으로 시작한다', async () => {
    const run = await createRun({ title: 'XBS 시작', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
  ...
  it('케이스명과 사전조건을 스냅샷으로 복사해 나중에 이름이 바뀌어도 그대로다', async () => {
    const run = await createRun({ title: 'XBS 스냅샷', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
  ...
  it('제한 시간을 안 주면 5분이 기본이다', async () => {
    const run = await createRun({ title: 'XBS 기본 제한', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
    expect(run.items[0]?.timeoutMs).toBe(300_000);
  });

  it('카탈로그에 없는 케이스는 그 tcId를 사유에 담아 거절한다', async () => {
    await expect(
      createRun({ title: 'XBS 없는 케이스', triggeredBy: 'tester', items: [{ ...항목, tcId: 'XBS-404', platforms: ['desktop'] }] }),
  ...
  it('빈 목록은 거절한다', async () => {
    await expect(createRun({ title: 'XBS 빈 목록', triggeredBy: 'tester', items: [] })).rejects.toThrow();
  });
  ...
  it('결과를 받으면 판정·소요시간·끝난 시각을 채운다', async () => {
    const run = await createRun({ title: 'XBS 결과', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
  ...
  it('절차와 검증 문장을 그대로 저장한다', async () => {
    const run = await createRun({ title: 'XBS 절차', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
    const historyId = run.items[0]!.historyId;
    await finishItem(historyId, { ...결과, historyId });
  ...
  it('러너가 죽은 항목은 NA와 사유로 남는다', async () => {
    const run = await createRun({ title: 'XBS 고장', triggeredBy: 'tester', items: [{ ...항목, platforms: ['desktop'] }] });
    const historyId = run.items[0]!.historyId;
  ...
  it('항목이 하나라도 안 끝났으면 실행 묶음은 FINISHED가 되지 않는다', async () => {
    const run = await createRun({ title: 'XBS 미완', triggeredBy: 'tester', items: [{ ...항목, platforms: [...항목.platforms] }] });
  ...
  it('항목이 전부 끝나면 FINISHED가 된다', async () => {
    const run = await createRun({ title: 'XBS 완료', triggeredBy: 'tester', items: [{ ...항목, platforms: [...항목.platforms] }] });

...
import { scan, type Duplicate } from './scanner.js';
...
async function runScan(log: FastifyBaseLogger): Promise<LastScan> {
  ...
  try {
    ...
    const problems = [
      ...duplicates.map((d) => `tcId ${d.tcId}이 ${d.files[0]}와 ${d.files[1]}에 겹쳐 있다`),
      ...failures.map((f) => `${f.file}을 읽지 못했다: ${f.message}`),
    ];

    // 중복은 스캔 실패다. 어느 쪽이 진짜인지 모르는 채로 캐시를 덮어쓰면 목록이 거짓말을 한다 (SPEC §3.1)
    ...
    const saved = await save(specs, failures.length === 0);
    last = {
      scannedAt,
      ...saved,
      duplicates: [],

...
- **RED** — `.claude/scripts/ci-covers-tests.test.mjs` 를 새로 쓴다. `.github/workflows/ci.yml` 을
  읽어 아래 **넷을 전부** 단언한다. 지금은 Playwright 단계가 **아예 없어** 실패한다.
  1. Playwright 단계가 **있다** — 못 찾으면 **그 자리에서 실패한다** (주의 2. 조용히 통과 금지)
  2. 그 단계가 **`jobs.check` 아래에 있다** — ★ BLOCKER 2
  3. `tests/` 아래 폴더 중 **돌지도 면제되지도 않은 것**이 없다
  4. `ci.yml` 이 **`npm run check:workflow` 를 돈다** — ★ BLOCKER 1. 이 검사 자신이 돌아야 한다
- **GREEN** — `ci.yml` 의 기존 `check` job 에 단계 셋을 더한다. ① chromium 내려받기
  ② `npx playwright test tests/todo --project=desktop` ③ **`npm run check:workflow`**.
  그리고 검사기의 면제 목록에 `demo` 를 **이유와 함께** 적는다.
  면제는 **그 검사 파일 안 한 곳**에만 둔다.
- **REFACTOR** — 검사 파일 머리에 **이 검사가 안 보는 것**을 적는다 (데스크톱만 돈다 ·
  케이스 안의 디바이스 선언은 안 본다). LEARNINGS 「검사기를 만들 때 무엇을 안 보는가를
  같이 적어 둔다」가 그렇게 하라고 적었다.

**files**: `.claude/scripts/ci-covers-tests.test.mjs`, `.github/workflows/ci.yml`
**depends-on**: []
**검증**: `npm run check:workflow` 가 EXIT=0. 그리고 **부러뜨려 확인한다** —
`ci.yml` 에서 ⒜ `tests/todo` 를 `tests/없는것` 으로 ⒝ Playwright 단계를 새 job 으로 옮겨
각각 EXIT≠0 이 되는 것을 보고 되돌린다

> **★ 왜 「검사가 자기를 돌게 하는 단계」까지 더하나 (BLOCKER 1).**
> 실측 — `check:workflow` 는 **검사에도 pre-push 훅에도 없다.** 사람이 손으로 칠 때만 돈다.
> 그대로 두면 이 할 일이 만든 장치가 **아무 데서도 자동으로 안 돌고**, 새 폴더가 생겨도
> 아무도 못 막는다. **구멍을 막으려고 만든 장치가 같은 구멍에 빠진다.**
> 실측 — `check:workflow` 는 **90초** 걸린다 (`# pass 70 · fail 0`).
...
### 할 일 4. 구멍이 실제로 닫혔는지 잰다 — **증명을 둘로 가른다**

> **★ 원안은 이 순서로 실행할 수 없다 (BLOCKER 3).**
> 「깨뜨려 푸시하고 검사가 빨개지는 것을 본다」였는데, **초안 PR 에서는 검사가 아예 안 돈다**
> (`.github/workflows/ci.yml` 이 초안을 통째로 건너뛴다). 체인은 마지막 단계 전까지 초안을 유지한다.
> 그래서 **[5] 에서는 한 줄도 못 돌린다.** 그리고 깨진 커밋을 브랜치 이력에 남기지 않는 편이 낫다.

- **RED** — **이 기계에서** `tests/todo/TODO-003` 의 기대값을 일부러 틀리게 바꾸고
  `ci.yml` 이 칠 **바로 그 명령**을 손으로 돌려 **EXIT≠0** 인 것을 본다.
  같은 명령이 지금 검사에는 없으므로 이것이 「구멍이 있었다」의 증거다.
- **GREEN** — 되돌리면 **EXIT=0**. 그리고 **배선은 [7] 이 증명한다** —
  초안을 풀면 도는 **첫 진짜 검사 실행**에 Playwright 단계가 있고 초록인 것을,
  `tpx-merge` Step 2 가 이미 보는 **실행 번호**로 확인한다.
- **REFACTOR** — **반드시 되돌린다.** `git status --porcelain` 이 비어야 한다

**files**: `tests/todo/TODO-003.spec.ts` (일시적. 되돌린다)
**depends-on**: [1]
**검증**: `git diff origin/main...HEAD -- tests/` 가 **비어 있다**.
게이트 2 요약에 **깨뜨렸을 때 EXIT≠0 · 되돌렸을 때 EXIT=0** 을 싣고,
[7] 에서 **새 실행 번호와 초록**을 싣는다
...
- **「데스크톱만 돈다」를 기계로 막는 것** (계획 검토 주의 3) — 앞으로 `tests/todo` 에
  모바일 케이스가 생기면 검사가 조용히 건너뛴다(실측 — 모바일로 돌리면 **15 skipped · EXIT=0**).
  싼 방어가 있지만 **지금 그런 케이스가 0건**이라 BLOCKER 셋을 먼저 닫는다.
  검사 파일 머리에 「안 보는 것」으로 적어 둔다
...
### 🛑 게이트 1 결과 — **지적 반영하고 진행** (2026-09-21)

사용자가 「이어서 진행하자」로 받았다. **BLOCKER 셋을 전부 계획에 반영했다** —
할 일 1 에 ①검사 자신을 돌게 하는 단계 ②`jobs.check` 안에 있는지 단언 ③못 찾으면 실패,
할 일 4 를 **증명 둘로 가르기**(빨강은 이 기계에서 · 배선은 [7] 에서).

**주의 3(「데스크톱만」을 기계로 막기)은 안 넣었다** — 지금 모바일 케이스가 0건이고
BLOCKER 셋을 먼저 닫는 것이 낫다. §「안 건드리는 것」에 남긴다.

**그 사이에 `main` 이 22커밋 움직여** 병합해 들여왔다 (되감기는 강제 푸시가 필요해 금지).

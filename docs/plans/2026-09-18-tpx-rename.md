...
## 확인 방법 — 세션을 새로 열어야 보인다 (검토 주의 1)

**병합 직후 `/tpx` 를 쳐도 안 뜬다.** 스킬 목록은 세션을 열 때 한 번 찍은 사진이다.
이번 세션에서 실측했다 — 디스크에 작업방이 **0개**인데 로더는 어제 지운 **3개**를 아직 안다.

순서는 이렇다.

1. 병합하고 `git pull` (체인이 자동으로 한다)
2. **Claude Code 세션을 새로 연다**
3. `/tpx` 를 친다 → 목록 맨 위에 체인이 뜬다
...
### 주의 1 — 병합해도 `/tpx` 가 바로 안 뜬다. 확인 방법이 그걸 안 적었다

**스킬 목록은 세션 시작 시점의 스냅샷이다.** 이번 세션에서 실측했다.
...
셋 다 어제 지운 것이다. 로더는 아직 안다. **디스크가 바뀌어도 색인은 안 따라온다.**

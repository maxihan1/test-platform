...
**측정값 — 2026-09-18, 이 기계 기준.** 설치된 명령 전체(내장 + 스킬 + 플러그인)를 대조했다.
플러그인 구성이 다르면 숫자도 달라지므로 **개수를 다른 문서에 옮겨 적지 않는다**(§2.7 ⑤).
...
`tpx` 가 앞으로도 0 이라는 보장은 없다. 지키는 기계 검사가 없으므로 **그날의 측정값**이다.
...
**depends-on**: []
**검증**: `ls` 에 `project-tp-chain.md` 가 없고 `project-tpx-chain.md` 가 있다 ·
`MEMORY.md` 링크가 새 파일을 가리킨다
(**`grep '/tp'` 가 0 이 되는 것을 조건으로 쓰지 않는다** — 그 파일이 「옛 이름이 `/tp` 였다」를
일부러 적고 있어 영원히 통과 못 한다. 처음에 그렇게 썼다가 code-review 가 잡았다)
...
여섯 자리 중 걸린 것은 **`docs/HOOKS.md` 와 `docs/WORKFLOW.md` 둘뿐**이고 할 일 4 가 맡는다.
나머지(킥오프 · spec-review · `check:*` · SETUP · DESIGN · 목업 · 코드 상수)는 등장 0건이다 —
스킬 이름은 로더가 디렉터리에서 읽지 코드가 복사해 두지 않는다.
...
**판정**: BLOCKER 1건 · 주의 2건 → 셋 다 반영함

| # | 지적 | 어디로 갔나 |
|---|---|---|
| BLOCKER 1 | 저장소 밖 기억 파일 넷이 `/tp` 를 가리킨다. 기억은 세션마다 자동으로 실리므로 **다음 세션이 같은 증상을 만난다** | → 할 일 6 신설 |
| 주의 1 | 확인 방법에 「안 뜨면 어떻게 하나」가 없다 | → 「확인 방법」 절 |
| 주의 2 | 할 일 4 검증이 `check:spec` 뿐인데 그 검사는 PR 본문을 안 본다 | → 할 일 4 검증 |

**주의 1 의 근거는 구현 중에 반쯤 틀린 것으로 드러났다.** 검토는 「스킬 목록이 세션 시작
스냅샷」이라고 봤는데(지운 작업방 셋이 남아 있는 것이 근거), 같은 세션에서 만든 `tpx` 여덟이
재시작 없이 그대로 떴다. **빼기만 안 따라온다.** 지적 자체는 유효했고 근거만 좁혔다.

검토가 확인한 것 — 할 일 1 의 RED 주장이 참 · 끝난 일의 기록을 안 고치는 판단이 옳음 ·
`tpx` 충돌 0 이 실측 · frontmatter `name:` 을 할 일 2 가 챙김 · `settings.json` 과 훅은 무관.

## 구현 후 code-review (2026-09-18)

지적 **30건** (in-diff 12 · pre-existing 18). 이 PR 에서 처리한 것과 넘긴 것은
게이트 2 요약과 `docs/reviews/2026-09-18-tpx-rename.md` 에 있다.

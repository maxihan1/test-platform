...
원인:  자동완성이 **부분열**로 찾는다 — 순서만 맞으면 매치라 `teleport` 의 `t`…`p` 가 걸린다.
       설치된 명령 전체와 대조하니 `/tp` 는 **52개**, `tpx` 는 0개였다
해법:  `(?<![a-zA-Z-])tp(?![a-zA-Z])` 한 줄. **이 항목은 옛 이름을 일부러 적고 있다** — 다시 돌리면 망가진다
교훈:  **새 슬래시 이름은 설치된 명령 전체와 부분열로 대조한다.** 앞글자 비교로는 못 잡는다
주의:  **저장소만 고치면 절반이다** — `~/.claude/…/memory/` 의 옛 이름이 다음 세션을 되돌린다

## [공통] 2026-09-18 · 개명하니 열려 있던 PR 의 상황판이 둘로 늘었다
증상:  `pr-update.mjs` 를 돌리자 PR 본문에 「진행 상태」 블록이 **두 개** 생겼다
원인:  블록을 찾는 마커가 `<!-- tp:status -->` 였다. **기계가 쓰는 닻에 사람이 읽는 이름이 박혀 있었다**
해법:  고아 블록을 손으로 걷어냈다. 열려 있던 PR 이 하나뿐이라 그걸로 끝났다
교훈:  **닻 문자열에 이름을 넣지 않는다.** 아직 `tpx:status` 다 — §2.5 기준 첫 번째라 기록만 한다.
       개명 전에 **열린 PR 이 있는지 먼저 본다**

## [공통] 2026-09-18 · 스킬 색인은 더하기는 따라오고 빼기는 안 따라온다
증상:  어제 지운 작업방 **3개**가 스킬 목록에 계속 떴다. 같은 세션에서 새로 만든 `tpx` 여덟은 **바로 떴다**
원인:  색인이 새 스킬은 다시 훑어 잡는데 사라진 것은 안 지운다
교훈:  **개명 뒤 새 이름이 안 뜨면 그때 세션을 새로 연다.** 대개는 그냥 뜬다 —
       「안 보이니 실패했다」고 먼저 단정하지 않는다. 반대로 지운 스킬이 남아 있어도 놀라지 않는다
주의:  처음엔 「세션을 새로 열어야 보인다」고 적었다가 **같은 세션에서 뜨는 것을 보고 고쳤다**

...
주의:  **검사 능력이 하나 줄었다.** `WORKSTREAMS.md` 킥오프 절 제목은 여전히 중요하다 —
       `tp-plan` 의 작업 원천이다. 없어진 것은 그 제목을 **기계가 대조할 근거**이지
       제목의 중요성이 아니다. 지금은 사람이 본다 (`tp-spec` §2.7 ④ 에 「기계가 안 본다」로 적었다)
...
해법:  계획 검토가 BLOCKER 로 잡았다. `[7] tp-merge` 의 정리 항목으로 옮겼다
교훈:  **git 정리는 할 일이 아니라 단계다.** `tp-merge` Step 1 의 「미커밋·미푸시 0 확인」이

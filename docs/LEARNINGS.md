...
원인:  Claude Code 자동완성이 **부분열**로 찾는다 — 글자가 순서대로만 나오면 매치다.
       `teleport` 안에 `t`…`p` 가 있다. 설치된 명령 전체와 대조하니 `/tp` 는 **52개**와 겹쳤다
       (`desktop` `output-style` `babysit-prs` `type` …). `tpx` 는 0개다
해법:  스킬 여덟을 `tpx` 접두사로 옮겼다. 치환은 `(?<![a-zA-Z-])tp(?![a-zA-Z])` 한 줄로 했다 —
       앞 조건이 `http`·`output` 과 옛 계획서 경로(`…-18-tp-legacy-purge.md`)를 같이 막고,
       뒤 조건이 `tpx` 재적용을 막아 **멱등**이 된다
주의:  이 항목은 **옛 이름을 일부러 적고 있다.** 같은 치환을 이 파일에 다시 돌리면 망가진다 —
       한 번 더 개명할 일이 생기면 기록 절은 손으로 확인한다 (이번에 실제로 한 번 망가뜨렸다)
교훈:  **슬래시 이름을 지을 때 설치된 명령 전체와 부분열로 대조한다.** 앞글자 비교로는 못 잡는다.
       그리고 **저장소만 고치면 절반이다** — `~/.claude/…/memory/` 가 세션마다 자동으로 실리므로
       거기 적힌 옛 이름이 다음 세션을 같은 자리로 되돌린다. 계획 검토가 이것을 BLOCKER 로 잡았다

## [공통] 2026-09-18 · 개명하니 열려 있던 PR 의 상황판이 둘로 늘었다
증상:  `pr-update.mjs` 를 돌리자 PR 본문에 「진행 상태」 블록이 **두 개** 생겼다. 덮어써야 하는데 덧붙었다
원인:  블록을 찾는 마커가 `<!-- tp:status -->` 였다. **기계가 쓰는 닻에 사람이 읽는 이름이 박혀 있었다.**
       이름을 바꾸니 옛 블록을 못 찾고 앞에 새로 붙였다
해법:  PR #20 본문에서 고아 블록을 손으로 걷어냈다. 개명 시점에 열려 있던 PR 이 하나뿐이라 그걸로 끝났다
교훈:  **닻 문자열에 이름을 넣지 않는다.** 마커·앵커·키는 사람이 안 읽으므로 이름을 따라갈 이유가 없다.
       지금은 `tpx:status` 로 여전히 이름이 박혀 있다 — `CLAUDE.md §2.5` 기준 **첫 번째**라 기록만 한다.
       **두 번째 개명이 오면** 마커를 이름과 무관하게(`chain:status` 같이) 바꾼다
주의:  개명 전에 **열린 PR 이 있는지 먼저 본다.** 있으면 그 본문을 손으로 옮겨야 한다

## [공통] 2026-09-18 · 스킬 목록은 세션을 열 때 찍은 사진이다
증상:  디스크에 작업방이 **0개**인데 스킬 로더는 어제 지운 **3개**를 아직 알고 있었다
원인:  스킬 색인이 세션 시작 시점 스냅샷이다. 디스크가 바뀌어도 안 따라온다
해법:  없다. 고칠 것이 아니라 **알고 있어야 하는 것**이다
교훈:  **스킬을 더하거나 이름을 바꾼 뒤에는 세션을 새로 열어야 보인다.**
       안 그러면 「개명이 실패했다」고 오판한다. 반대로 지운 스킬이 목록에 남아 있어도 놀라지 않는다

## [공통] 2026-09-18 · 체인에 「기계적 변경」 통로가 없다
증상:  `/tpx` 개명은 `sed` 한 줄인데 3등급이 나왔다. `tpx-impl` 은 할 일마다 하위 작업자를 띄우라고 한다
원인:  등급을 **경로**로 정한다. 색인 `docs/SPEC.md` 에 체인 설명 두 줄이 있어서 `SPEC` 표면에 걸렸다.
       바꾸는 내용이 산문 한 낱말이어도 표면은 같다
해법:  이번엔 인라인으로 했다. 규율(RED→GREEN·종료 코드 판정·계획 대조)은 그대로 지켰다
교훈:  **등급은 「어디를 건드리나」를 재지 「얼마나 위험한가」를 재지 않는다.** 대체로 맞는 근사지만
       기계적 개명에서는 어긋난다. 지금은 게이트에서 사람이 조정한다 — **두 번째가 오면**
       `surfaces.mjs` 에 「치환만 하는 변경」 예외를 두는 것을 검토한다 (§2.5 승격 규칙)

...
5. **리눅스 서버면** `HOST_UID`·`HOST_GID` 에 저장소 주인의 `id -u`·`id -g` 를 적는다 (맥은 비워도 된다) — 안 맞으면 컨테이너가 만든 파일 때문에 사람이 `git pull` 을 못 한다
6. 켠다 — **저장소 뿌리에서**:
...
**남는 한계 — 믿을 수 있는 기획서만 넣는다.** 자식은 같은 프로세스 사용자라 에이전트의 환경(GitHub·Claude·에이전트 토큰)을 `/proc` 로 읽을 수 있고,
같은 HOME 을 써서 전역 git·claude 설정을 다음 건에 물려줄 수 있다(컨테이너를 다시 켜면 HOME 을 비운다),
**서버 저장소 전체에 쓸 수 있다** — 작업방 밖의 `tests/` 를 고치면 PR 없이 목록에 뜨고 실행된다. **고객사에 설치하기 전에 격리 후속 PR 이 필요하다** (`docs/spec/도메인/작성.md` §3.6).
켜기 전에 저장소 뿌리에 `.env` 가 있어야 한다 — 없으면 docker 가 가림 자리를 root 소유 빈 파일로 만든다.
한 번에 한 건씩 돈다 — 머지가 CI 를 최대 17분 기다리는 동안 다음 작성이 선다. 구독 한도를 사람의 대화형 사용과 나눠 쓴다.
Claude 토큰은 1년이면 만료된다 — 그때 3번을 다시 한다. 대상 서버 주소가 `localhost` 면 컨테이너 안에서 자기 자신이라 실행 관문이 실패한다.

**author 가 도는 동안 호스트에서 `git worktree prune` 을 치지 않는다** — 컨테이너가 만든 작업방 기록은 `/repo/…` 경로라
호스트 git 눈에는 없는 폴더로 보여 지워진다. 작업방에서 compose 를 띄우지도 않는다 — 저장소 뿌리에서만.

...
새 스킬(`tpx-author`)과 새 pre-push 훅이 main 에 있어야 돈다. 특히 **훅은 main 체크아웃의 것이 돈다** —
작업방에서 push 해도 그렇다(`core.hooksPath` 가 main 체크아웃을 가리킨다). **pull 을 안 하면 옛 훅이
케이스만 바뀐 push 에도 검사 기록을 요구해 PR 이 안 열린다.** 아래 두 줄을 main 체크아웃에서 친다.

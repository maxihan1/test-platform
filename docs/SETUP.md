...
> **★ 이것은 「지금 도는 방식」이다. 명세가 가는 곳은 다르다** (2026-09-22).
>
> 여기 적힌 것은 **사람이 셸에서 직접 부르는 길**이다 — 기획서를 **맥의 파일 경로**로 읽고
> **admin 서버를 아예 안 부른다.** 그래서 **로그인을 지나지 않는다.**
>
> 명세([`docs/spec/도메인/작성.md`](spec/도메인/작성.md))가 정한 것은 **화면 대기줄**이다 —
> 화면이 요청을 넣고 에이전트가 **계정으로 로그인해** 집어 간다. 기획서 본문도 파일이 아니라
> 대기줄 행에 실려 온다.
>
> **둘이 같이 살면 안 되는 이유** — 대기줄 통로를 만든 뒤에도 이 셸 진입점을 그대로 두면
> **「로그인을 안 지나는 작성 경로」가 남는다.** 그것은 명세가 러너 포트를 닫으며 막은
> 뒷길과 같은 성질이다.
>
> **★ 맥이 대기줄을 쓰려면 서버에 이름을 알려 줘야 한다 (2026-09-22).**
> `AUTHORING_AGENT_USER` 에 **맥이 쓰는 계정 아이디**를 적는다. 그 이름이 아니면
> 집기·단계 올리기·사진 올리기·끝내기 넷이 전부 **403** 이다.
> **안 적으면 아무도 못 집는다** — 열어 두면 그 서비스에 배정된 아무 실행 등급 계정이
> **맥보다 먼저 집어 병합될 PR 주소를 자기 것으로 바꿔치기할 수 있다.**
> 그 계정은 **`operator` 여야 한다. `admin` 을 주지 마라** — 맥에 저장된 비밀값 하나가
> `/api/settings/**` 전부를 연다.
>
> **★ 판단이 났다 (2026-09-22) — 없앤다. 다만 이 PR 이 아니라 그다음 PR 이다.**
> 대기줄 표와 통로는 2026-09-22 에 섰지만 **화면도 없고 에이전트도 아직 대기줄을 모른다.**
> 지금 없애면 **이 기능이 아예 안 돈다** — 기획서를 넣을 길이 하나도 없어진다.
> **에이전트를 대기줄에 붙이는 PR 이 이 문단과 아래 명령을 같이 치운다.**
> 그전까지는 **이 문단을 지우지 마라.**

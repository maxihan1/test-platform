...
- 막힌 것: 없음. 다만 **검사가 1회는 통과하고 2회부터 깨졌다** — 원인은 내 검사가 만든 서비스 넷을
  안 치운 것이었다. **연속 3회 규칙(G3)이 잡으라는 바로 그 경우**이고 1회만 돌렸으면 못 봤다.
...
`check:deps` · `typecheck` · `check:spec` · `check:tests` · `check:secret-names` ·
`check:workflow` 전부 **EXIT 0** · **`npm test` DB 붙여 연속 3회 `0·0·0`** —
파일 89개 · 검사 **1110개** · **건너뜀 0건**.

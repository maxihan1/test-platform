...
-- 맥은 컨테이너 밖이라 러너처럼 같은 볼륨을 보지 못한다 — 오가는 것이 전부 이 표를 지난다
CREATE TABLE authoring_request (
  ...
  screenshot_dir    TEXT,                   -- 이 요청의 사진이 들어간 폴더. 무엇으로 가를지는 아직 안 정했다
...
| `spec_text` | `authoring_request` | 기획서 경로만 남는다. **맥은 다른 기계라 그 경로를 못 읽고**, 그 파일이 고쳐지면 무엇을 시킨 요청이었는지도 같이 바뀐다 |
...
컨테이너 안 명령으로 풀린 것과 다른 점이다). 그래서 기획서 본문도 테스트 소스 본문도 이 표를 지난다.
...
| 무엇 | 정한 것 |
|------|--------|
| `status` | **`PENDING`·`RUNNING`·`DONE`·`FAILED` 넷.** 전이는 `PENDING→RUNNING`(집기)과 `RUNNING→DONE\|FAILED`(끝내기) **둘만** 허용하고 그 밖은 `409`다. 이 규칙 하나가 셋을 닫는다 — 끝난 행에 「끝났다」가 또 와서 판정이 덮어써지는 것 · 집기가 `PENDING`만 집으므로 `claimed_by`가 덮어써질 일이 구조적으로 없는 것 · 머지가 아직 안 끝난 요청을 가리키는 것 |

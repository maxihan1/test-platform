...
**셋째가 다음 PR이다.** 명세에 표가 있는데 `db/`에 없는 것은 버그가 아니라 순서다.
그 마이그레이션이 지킬 것 둘.
...
| 무엇 | 지금 |
|------|------|
| `status` 값이 위 목록으로 충분한가 | **화면을 그려 봐야 안다.** 목록이 정본이고 개수는 세어 적지 않는다 |
| `screenshot_dir`를 무엇으로 가르나 | 러너는 `runs/{runId}/{historyId}`로 가르는데 **머지 전 실행에는 그 행이 없다.** 대기줄 행 번호가 가장 가깝지만 **아직 안 정했다.** 안 정하면 모든 요청이 같은 폴더에 겹쳐 쓰고 직전 요청의 사진을 덮는다 |

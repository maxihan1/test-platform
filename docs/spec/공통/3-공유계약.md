...
| 검색 조건 | 케이스 속성(글자·디바이스·활성)은 **서버가**, `마지막 결과`는 **화면이** 거른다. 서비스는 조건이 아니라 **맨 위 띠의 선택**이고 서버가 늘 적용한다 (2026-09-17 수정 — 목록이 한 서비스로 좁혀지면 그 안의 접두사는 전부 같아서 거를 것이 없다). 조건 정의는 §8.1이 정본이다 | 마지막 결과는 실행할 때마다 바뀐다. 카탈로그가 `run_item`을 읽으면 컨텍스트 경계가 깨진다 (§3.1) |

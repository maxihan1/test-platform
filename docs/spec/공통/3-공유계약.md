...
| `GET /api/runs/last-by-case` | 끝난 항목만, 케이스×디바이스마다 최신 1건. **최신은 「가장 마지막 회차」다** — 반복 5회 중 3승 2패면 마지막 회차의 판정이 그대로 배지가 된다. 승패를 요약하지 않는다 (2026-09-17 확정. 회차별 승패는 §8.3 회차 요약에서 본다) | §8.1 목록의 `마지막 결과` 칸 때문이다. 케이스마다 이력을 따로 부르면 10건일 때 요청이 11번이 된다. 카탈로그가 `run_item`을 읽으면 컨텍스트 경계가 깨지므로 실행 쪽이 낸다 |

...
상대경로로 걸면 작업방마다 그 작업방의 훅이, 절대경로로 걸면 늘 main 체크아웃의 훅이 돈다 (아래 「차선」 절).
...
**예외 — 문서만(`docs` 차선) · 케이스만(`cases` 차선) 바뀐 push 와 바뀐 파일이 0 인 push 는 기록을 요구하지 않는다** (아래 「차선」).
명세가 바뀐 push(`spec` 차선)는 기록을 요구한다 — 명세 PR 의 유일한 사람 검사가 spec-review 다.

**pre-push 는 검사를 부르기 전에 `GIT_*` 환경 변수를 걷는다** (2026-09-25). git 이 훅에 넣어 주는 `GIT_DIR` 등을 검사가 물려받으면
임시 폴더에 git 저장소를 만드는 검사가 **이 저장소**에 커밋하고 공유 `.git/config` 에 `core.bare = true` 를 박는다 — 두 번 났다(LEARNINGS 2026-09-24).
`hook-contract.test.mjs` 「훅은 검사를 부르기 전에 GIT_* 환경 변수를 걷는다」가 지킨다. **맨 셸에서 `npm test` 를 돌릴 때는 이 보호가 없다** — 새 검사는 여전히 git 을 부를 때 `GIT_*` 를 빼고 부른다.
...
| 무엇을 보나 | |
|---|---|
| 타입 · 단위 테스트 · 테스트 코드 **형식** 규칙 · SPEC 문서 · 이름 갈림 둘 | 전부터 있던 것. 단위 테스트는 2026-09-25 부터 **바뀐 것과 이어진 것 1회 + `test:always`** (아래 「차선」) |
...
- **한 번만 돈다.** 여러 번 돌리는 것은 **제품의 실행 요청이 맡는다** (`playwright.config.ts` 주석)
- **base 이후 바뀐 케이스만 돈다** (`--only-changed`, 2026-09-25). 손대지 않은 케이스는 CI 에서 다시 안 돈다
...
### 차선 — 바뀐 만큼만 검사한다 (2026-09-25, 정본은 여기다)

문서 한 줄을 바꿔도 DB·브라우저·전체 단위 테스트 3회가 돌아 작업이 느렸다 — 빈 시작 커밋 push 에도 58초가 들었다.
**서비스 전이라 회귀 그물을 좁혀도 된다**는 판단으로 바뀐 파일을 네 차선으로 가른다(사용자 결정 2026-09-25).
판정은 `.claude/scripts/lane.mjs` 한 곳이다. **훅과 CI 가 같은 것을 부른다.** `detect-tier.mjs` 도 `차선:` 을 찍는다.

| 차선 | 언제 | CI | pre-push | 검사 기록 |
|---|---|---|---|---|
| `docs` | 전부 `docs/**` 또는 루트 `*.md`, 명세 없음 | `check:spec` 만. **설치도 안 한다** | `check:spec` | 요구 안 함 |
...
| `full` | 그 밖 (코드 · 스킬 · CLAUDE.md · 명세+코드 · 미분류) | 전부. 단위 테스트는 **바뀐 것과 이어진 것 1회** + `test:always`, 케이스는 **바뀐 것만** | 같음 (migration·설정이 바뀌면 전체) | 요구한다 |
...
- **`test:always`** — `vitest --changed` 는 import 로 이어진 검사만 고른다. 파일을 직접 훑는 검사(모든 route 에 인증이 붙었나 등)는
  못 골라서 따로 매번 돈다. 목록은 `package.json`, 빠진 것은 `always-tests.test.mjs` 가 잡는다
- **migration·설정 변경** — CI 는 `vitest.config.ts` 의 `forceRerunTriggers` 로 전체를 돈다. **작업방(`.claude/worktrees/`)에서는
  그 트리거가 안 걸린다** — 경로에 점 폴더가 끼면 vitest 의 패턴이 빗나간다(2026-09-25 실측). 그래서 훅은 파일 이름을 직접 보고 `npm test` 전체를 돈다
- **걷어 낸 그물** — 단위 테스트 연속 3회(찌꺼기 fixture 가 2회차부터 깨지는 것을 잡았다). 서비스를 열 때 되살린다
- **훅은 `core.hooksPath` 가 가리키는 체크아웃의 것이 돈다** (아래). 절대경로로 걸린 맥은 병합 뒤 main 에서 `git pull` 해야 새 차선이 먹는다

### 테스트만 바뀐 PR·push 는 가벼운 길 — 2026-09-23 신설, 지금은 `cases` 차선

작성 에이전트가 여는 PR 은 케이스 파일만 바꾼다. 코드가 한 줄도 안 바뀌었는데 DB·단위 테스트까지 다 돌면
시간만 든다. 그래서 **바뀐 파일이 케이스뿐이면 CI 와 pre-push 훅이 둘 다 가벼운 길로 간다.**
에이전트가 무엇을 어떤 순서로 하는지는 `docs/spec/도메인/작성.md` §3.6 「작성 에이전트는 전용 체인이다」가 정본이다.
...
- 가벼운 길 — 바뀐 파일(지운 것 포함)이 **전부** `tests/<폴더>/*.spec.ts` 이거나 `docs/cases/*.md`.
  **새 서비스 폴더도 된다** (2026-09-25 게이트 1) — 앞 판은 「CI 실행 단계에 그 폴더가 없어 다음 무거운 PR 이 빨개진다」로
  ...
  그래서 `ci-covers-tests` 가 **`<접두사>-NNN.spec.ts` 만 든 폴더**를 서비스 폴더로 보고 면제한다. 도우미 파일이 섞인 폴더는 여전히 사람이 CI 에 넣거나 사유를 달아 면제한다
...
| 돈다 | 빠진다 |
|---|---|
| 타입 검사 · 테스트 코드 규칙(`check:tests`, K 규칙) · 비밀값 이름(`check:secret-names`) · `check:spec` | DB 준비·마이그레이션 · 브라우저 내려받기 · 단위 테스트 · `check:evidence-formats` · `check:workflow` · **케이스 실행** |

**★ CI 는 새 케이스를 실행하지 않는다.** 사내 대상이나 비밀값이 필요한 케이스를 CI 러너가 못 돌리기 때문이다.
**병합 근거는 에이전트가 PR 본문에 싣는 관문 3(3회 실행) 기록**이다. 가벼운 길의 초록불은 「형식이 맞다」까지만 말한다.
...
**작성 에이전트의 push 에는 훅이 아예 안 돈다** (2026-09-24) — 에이전트는 작업마다 만든 사본에서 `core.hooksPath=/dev/null` 로 push 한다.
사본 트리의 훅은 자식(기획서를 읽은 Claude)이 쓸 수 있는 자리라 에이전트 권한으로 돌리면 안 된다. 같은 검사(타입·K 규칙)는 CI 의 가벼운 길이 한다.
사본 트리에는 `origin/main` 도 없어 「이미 있던 케이스에 새 미확정 꼬리표」 경고(도메인/실행 §3.2)는 거기서 건너뛴다 — CI 로그에 뜨는 그 경고를 게이트에서 사람이 본다 (2026-09-25).
**아래 두 줄은 사람이 push 할 때의 이야기다.**
...
에이전트는 저장소 주인 계정으로 돈다. **막는 것은 에이전트의 판정(`check` 가 `success` 일 때만 병합)과 「관리자 우회를 쓰지 않는다」 둘뿐이다.**

**신뢰 경계** — 기획서를 올릴 수 있는 사람(그 서비스의 `operator`)의 파일은 자식 Claude 가 셸 권한으로 읽는다.
자식은 **GitHub 자격증명이 없는 환경**이라 push·병합을 못 하고, 에이전트가 올리기 전에 **진짜 main**(`git ls-remote` 의 SHA) 기준으로
「테스트만」을 판정한다. **서버 컨테이너에서는** 자식이 다른 uid 라 에이전트의 사본 git·서버 저장소에 못 쓴다 (2026-09-24).
**맥에서는** 자식이 같은 uid 라 맥의 파일과 사본의 bare git 설정(filter·credential.helper)에 쓸 수 있다 — 훅은 끄고 push 한다. 키체인과 함께 전용 macOS 계정으로 푼다(후속).
정본은 `docs/spec/도메인/작성.md` §3.6 「작성 에이전트는 전용 체인이다」.
...
**알려진 한계 둘** — 가벼운 길은 `tests/<폴더>/` 에 들어온 spec 을(새 서비스 폴더 포함) **CI 에서 실행하지 않는다.**
2026-09-25 까지는 그런 케이스가 **다음 무거운 PR 에서** 실행돼 빨개질 수 있었다. 지금은 무거운 PR 도
**base 이후 바뀐 케이스만** 돌리므로(`--only-changed`) 남의 PR 로 번지지 않는다. 대신 그 케이스는 **CI 에서 한 번도 안 돈다** —
근거는 에이전트의 관문 3 기록뿐이다.
...
- `tests` 훅을 `npm run check:tests`로 교체 — WS-A가 검사기를 만든 뒤. 지금 정규식은 Claude 편집 전용
  1차 방어이고, 사람이 쓴 테스트 코드는 CI가 같은 규칙(SPEC §4 K표)으로 검사한다

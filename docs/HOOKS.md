...
맥 작성 에이전트는 이 파일 하나를 복사해 **병합 직전 판정**에 쓴다. **그래서 이 파일은 다른 파일을 import 하지 않는다.**
...
자식은 **GitHub 자격증명이 없는 환경**이라 push·병합을 못 하고, 맥이 올리기 전에 **진짜 main**(`git ls-remote` 의 SHA) 기준으로
...
정본은 `docs/spec/도메인/작성.md` §3.6 「맥 경로는 전용 체인이다」.

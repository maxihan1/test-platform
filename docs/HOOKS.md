...
**`/tpx` 로 도는 작업도 이 파일에 넣어야 한다.** 2026-09-18 까지 이 자리에 「`tpx-start` 가 `export` 로 세운다」고 적혀 있었는데, 바로 위 문단이 말하듯 `export` 는 안 먹는다 — 그동안 `ownership` 검사는 켜진 적이 없다. 갈래가 없는 작업(하네스·계약 반영)에는 아예 세우지 않는다 — 세우면 못 고칠 파일이 생긴다.

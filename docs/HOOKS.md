...
| `cases` | 아래 「가벼운 길」 그대로 | 타입 · K1~K10 · 비밀값 이름 · `check:spec` | 타입 · `check:tests` | 요구 안 함 |
...
| 돈다 | 빠진다 |
|---|---|
| 타입 검사 · 테스트 코드 규칙(`check:tests`, K1~K10) · 비밀값 이름(`check:secret-names`) · `check:spec` | DB 준비·마이그레이션 · 브라우저 내려받기 · 단위 테스트 · `check:evidence-formats` · `check:workflow` · **케이스 실행** |
...
사본 트리의 훅은 자식(기획서를 읽은 Claude)이 쓸 수 있는 자리라 에이전트 권한으로 돌리면 안 된다. 같은 검사(타입·K1~K10)는 CI 의 가벼운 길이 한다.

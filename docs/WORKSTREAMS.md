...
> **① 뼈대가 2026-09-19 에 끝났다** (PR #25). 아래 23개 중 **1 · 3 · 4 · 7 · 10 · 11 · 12 · 13 · 19** 아홉이다.
> **화면이 아예 안 뜨던 것을 되살렸다** — 로그인 문과 `?service=` 필수를 화면이 모르고 있었다.
> 남은 열넷은 두 덩이다.
>
> | 덩이 | 남은 항목 |
> |---|---|
> | **② 화면 고치기** (다음) | 2 총건수 페이지 계산 · 5 검색 조건 넷 · 6 대상 서버·반복·실행자 · 8 멈춤·회차 요약·파라미터 줄 · 9·17 증적 버튼과 목록 · 14 실행 기록 목록 · 16 빈 목록 세 갈래 · 21 완료 모달 · 22 Slack 체크박스 |
> | **③ 마감** | 15 설정 화면 · 18 좁은 화면 · 20 글꼴 번들 · 23 Slack 웹훅 칸 |
>
> **먼저 읽을 것** — `docs/progress/WS-E.md` 의 「다음 세션이 알아야 할 것」.
> 진입점이 바뀌었다(`main.tsx` → `Shell.tsx` → 화면), 순수 함수가 `layout.ts`·`mask.ts`·`role.ts`·`runState.ts` 로 갈렸다,
> 좁은 화면은 375px `iframe` 으로 본다(창 리사이즈가 안 먹는다).
>
> **② 가 이어받을 자리 둘** — 알림 줄이 지금은 「도는 중」만 말한다. 「끝났습니다」로 바뀌는 것과
> 완료 모달(§8.9)은 `layout.ts` 의 `알림줄()` 에 얹는다. 그리고 `triggeredByName` 이
> 응답에는 오는데 화면이 아직 아이디를 그린다 (킥오프 14).

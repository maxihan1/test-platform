...
6. 데모 테스트 10건 (SPEC §10) — 아직 defineCase 없이 순수 Playwright로
   ...
   DEMO-008~010이 환경 축 검증용이므로 빠뜨리지 마라
...
1. POST /api/runs — test_run 생성 후 run_item들을 만들고 디스패처에 넘김
   - 요청의 platforms 배열 길이만큼 run_item을 만든다 (케이스 1건 × 환경 2개 = 2행)
...
만들 것 (SPEC §8):
1. 케이스 목록 — TC ID / 문장형 케이스명 / 지원 환경 / 마지막 결과 / 실행 버튼 (SPEC §8.1)
   JSON 원문은 목록에 절대 노출하지 않는다
   목록 위에 '다시 스캔하기' 버튼과 마지막 스캔 결과·실패 사유 (GET /api/catalog/scan)
2. 실행 설정 — paramSchema를 읽어 입력 폼을 자동 생성
   enum→셀렉트, boolean→토글, optional→선택 입력, describe→한글 라벨
   저장된 ParamSet 불러오기 드롭다운. 검증 실패 시 버튼을 비활성화하지 말고
   해당 칸 아래에 이유를 표시한다 (SPEC §8.2, DESIGN.md)
3. 실행 결과 목록 — 케이스 1행에 환경별 판정을 나란히 묶어 표시
   상태 필터 + 환경 필터, 페이지네이션
4. 항목 상세 — SPEC §8.4 포맷 그대로
   실패한 검증 문장 아래에 스크린샷 썸네일, 그 아래 접힌 '실패 지점 코드'
   코드 뷰는 기본 접힘. 펼쳐야 보인다

2번의 폼 자동 생성이 이 플랫폼의 핵심 기능이다. 여기부터 만들어라.

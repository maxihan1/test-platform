...
- **WS-E(화면)** — 작성 화면의 「실제 화면과 대조」 체크박스 · 대상 서버 · 시작 주소 · 목록의 미확정 배지와 나이 · 결과 막대의 미확정 칸.
  ...
  **설정 화면의 계정 칸 (WS-F 뒤, 2026-09-26)** — 저장할 때 줄에 `loginId` 를 되돌려 보내고 비밀번호는 새로 넣을 때만 싣는다(안 보내면 서버가 유지한다) ·
  **키 이름을 고치면 그 줄의 계정이 비워진다고 알린다**(인증 §8.8) · 작성 화면은 계정 없는 줄을 **미리 거르지 않는다**(`hasLogin` 없음) — 고르면 서버의 400 `BAD_ENV` 를 「이 대상 서버에는 테스트 계정이 없습니다」 꼴로 보여 준다

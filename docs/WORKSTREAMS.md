...
## ⏸ 아직 갈래가 없는 일 — 「테스트 작성」 화면 (2026-09-21)

기획서에서 케이스와 테스트 코드를 만들고 화면에서 머지까지 하는 기능이다.
**아래 소유 경로 표에 자리가 없다** — 화면·서버·명세·맥 스크립트를 한꺼번에 건드려서 갈래 하나에 안 담긴다.

**어드민 화면 전면 리뉴얼이 끝난 뒤에 착수한다.**

**구조 결정 다섯과 근거는 [`docs/plans/2026-09-21-테스트작성-화면-설계.md`](plans/2026-09-21-테스트작성-화면-설계.md) 에 있다.**
그 문서를 안 읽고 시작하면 같은 결정을 처음부터 다시 하게 되고,
**명세 세 곳에 잠금이 걸려 있어 중간에 막힌다.**
...
> ① 드롭다운 비밀값 · ② 화면 그물 · ③ `aria-describedby` · ④ 모달 규칙 다섯 ·
...
5-3. **사이드바 첫 자리 이름은 `테스트케이스 목록` 이다** (2026-09-21 ②).
   목록의 정본은 SPEC 공통/5-화면공통 §「자리는 넷」이다
...
12. 자리 넷 — 케이스 / 실행 기록 / 그래프↗ / 설정. 사이드바에 세로로 선다. 도는 실행이 있으면 그 아래 카드 하나 (SPEC §8)

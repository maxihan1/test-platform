...
> **`RunSetup.test.tsx` 가 없다는 것은 ② 와 같은 자리다 — 새 번호를 만들지 않는다.**
> ② 가 「진짜 자리는 `RunSetup.tsx` 고 그물 밖이다」라고 적어 둔 그 파일이다.
> 이번에 그 파일을 한 줄 고쳤고 **타입 검사로 갈음했다** — 그 화면이 자기 디바이스 수를 넘기는지
> 무는 자리는 여전히 비어 있다. 나머지를 덮을 때 1순위라는 판단도 그대로다.
>

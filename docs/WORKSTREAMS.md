...
- **Grafana 「작성 현황」 — 대기 (2026-09-26 사용자 · WS-D 와 따로).** 서비스별 **가장 오래된 미확정의 나이** · **작성에 걸린 시간**(기획서를 넣고 초안 PR 이 나오기까지).
  **명세 §8.5 가 패널을 넷으로 정했고 `grafana_ro` 는 `test_run`·`run_item` 만 읽는다** — 나이는 `test_case.unconfirmed_since`, 걸린 시간은 `authoring_request` 가 필요해
  명세·권한(마이그레이션) 변경이다(게이트 0). 케이스 목록 머리에는 나이가 이미 보인다(WS-E ①).
  **같은 PR 에서 명세 두 줄을 보강한다**(PR #83 spec-review) — 리포팅 §8.4 「증적 문서 머리말」에 판정 한 줄(확정 칸 0 도 적음 · 항목 행 단위 · 묶음 꼴은 실행 §3.2),
  §7 insights 줄에 「직전 실행에서만 미확정이던 케이스는 새 케이스처럼 견주지 않는다」

...
- **WS-E ② — 작성 화면 (WS-작성 서버와 같이 또는 바로 뒤, 2026-09-26 사용자가 둘로 나눴다)** — 받아 줄 서버 통로가 WS-작성에서 생긴다.
  작성 화면의 「실제 화면과 대조」 체크박스 · 대상 서버 · 시작 주소 ·
  **작성 상세의 산출물(`role`) · 차이 목록(`diffs`) · 표시 실패의 「표시 못 함 — 이유」(`markError`)** — `markError` 표시는 명세가 아니라 이 갈래와 `docs/DESIGN.md` 로 넘겼다.
  작성 화면은 계정 없는 줄을 **미리 거르지 않는다**(`hasLogin` 없음) — 고르면 서버의 400 `BAD_ENV` 를 「이 대상 서버에는 테스트 계정이 없습니다」 꼴로 보여 준다.
  **역방향 요청에는 「다시 돌리기」를 내지 않는다**(서버가 409 `BAD_SOURCE` — 2026-09-26 WS-작성 ① 게이트 1). 대신 「역방향은 새 요청으로 넣어 주세요」를 보인다.
  서버 통로는 PR #79 로 섰다 — 만들기 본문 `compare·env·startUrl` · 목록·상세 같은 칸 · 상세 `assets[].role·sourceAssetId` · `result.diffs[]`(모양 검사 없음 — 없는 칸을 방어해 그린다)

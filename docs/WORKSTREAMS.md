...
**그 PR 은 명세만 세웠다.** 계약 블록은 전부 `상태: 대기` 였다 — 타입·DB 칸 둘은 PR #74, 집계는 PR #76, 설정 `envs[]` 는 PR #77, 작성 통로 · `outputs` 권한 세 줄은 PR #79 에서 `반영 완료` 가 됐다. 계약 블록 중 `대기` 는 더 없다 — 남은 것은 화면(WS-E ②)·에이전트(WS-작성 ③)·증적(WS-D)이 채우는 일이다.
...
- **WS-D — 반영 완료 (2026-09-26, PR #83).** 증적(HTML·PDF) 머리에 **판정 한 줄**(`통과 3 · 실패 1 · 미실행 0 · 미확정 2(통과 1 · 실패 1)` — 확정 칸은 0 도 적고, 묶음 글자는 화면 `web/unconfirmed.ts` 와 같다는 검사) ·
  본문은 확정 블록 → 「미확정 — 기획 답 대기」 묶음(블록 머리에 박제 사유, 판정 색 없음) · 엑셀 `미확정 사유` 칸(`미실행·판정불가 사유` 뒤, 모든 행) ·
  Grafana `성공률 추이`·`실패 TOP 10` 은 **접기 전에** `i.unconfirmed IS NULL`(평균 소요시간은 측정치라 그대로) ·
  insights 는 이번에 미확정인 케이스를 판정·**빠진 건수**·실패덩어리에서 빼고, 직전만 미확정이면 새 케이스처럼 둔다. 판정 줄은 `apps/admin/src/reporting/summary.ts`.
- **Grafana 「작성 현황」 — 반영 완료 (2026-09-26, PR #84).** 패널 `가장 오래된 미확정`(활성 서비스·활성 케이스, 나이는 카탈로그에 들어온 뒤) ·
  `작성에 걸린 시간`(최근 30일 끝난 처음 작성 요청, 서비스 × 보통/화면과 대조, 대기·작업 중간값과 실패 수). 규칙은 명세 리포팅 §8.5 「작성 현황」.
  `grafana_ro` 는 `test_case`·`service`·`authoring_request` 의 **칸 일부만** 읽는다 — 칸 목록 정본은 데이터모델 「대시보드 칸 권한」.
  같은 PR 에서 명세 두 줄 보강 — 리포팅 §8.4 머리말 판정 한 줄 · §7 insights 「직전만 미확정」
- **WS-E ① — 반영 완료 (2026-09-26, PR #78).** 서버가 이미 있는 화면. 실행 결과·실행 기록·진행 막대·완료 모달·알림 줄에 미확정 칸과
  `미확정 5(통과 4 · 실패 1)` 묶음(글자 규칙은 `apps/admin/src/web/unconfirmed.ts` 한 곳) · 미확정만 돌린 실행의 거터는 판정 색 없음 ·
  실행 결과 행에 `미확정 · <박제 사유>` · 케이스 목록 배지와 머리의 `미확정 N건 · 가장 오래된 것 D일째` ·
  설정 대상 서버 줄의 테스트 계정 칸(비밀번호 `설정됨 · 다시 넣기`, 안 건드리면 안 보냄, 키 이름을 바꾸면 비밀번호가 비워진다는 안내).
  화면 규칙은 `docs/DESIGN.md` 「판정 표기」 미확정 표, 목업에도 예시를 넣었다.
- **WS-E ② — 반영 완료 (2026-09-26, PR #80).** 작성 새 요청에 「실제 화면과 대조」 체크박스 · 대상 서버(띠의 서비스 `envs` 전부 — 계정 없는 줄도 거르지 않는다) · 시작 주소(비우면 기획서가 말하는 화면, 시작 주소만 있으면 기획서 없이 보낸다 = 화면만) ·
  서버 거절 `BAD_ENV`·`BAD_START_URL` 을 사람 말로 · 대상 서버가 0 이면 설정으로 안내 · 목록 줄에 `화면과 대조` ·
  상세에 대조 설정(`qa · <시작 주소>` · 화면만 표시) · 입력 자료와 **산출물**(표시 사본 — 원본 이름 · 역기획서)을 따로 · **차이 목록 표**(번호·종류·자리·기획서·화면·케이스·`표시함` / `표시 못 함 — <markError>`, 판정 색 없음).
  차이 목록은 서버가 모양을 안 봐서 화면이 방어해 읽는다(`web/authoringView.ts` 의 `차이목록`). **「다시 돌리기」는 원래 화면에 버튼이 없어 할 일이 없었다** — 서버가 역방향 원본의 재실행을 409 로 막는다.
  화면 규칙은 `docs/DESIGN.md` 「테스트 작성 — 역방향」, 목업에도 예시를 넣었다. **앱을 띄워 눈으로 보지는 못했다** — 역방향 요청이 끝난 기록이 아직 없다(에이전트 ③ 뒤)
- **WS-F — 반영 완료 (2026-09-26, PR #77).** 설정 `envs[]` 줄에 `loginId` · `loginPassword` 저장, 응답은 `loginId` · `hasLoginPassword` 만(원문은 목록·`/auth/me`·400 본문 어디에도 없다 — 검사가 지킨다) ·
  **두 칸 다 키 없음 = 유지, null·빈 글자 = 지움**(게이트 1 — 지금 설정 화면이 줄을 `{ env, baseUrl }` 로 잘라 보내 아이디가 지워지던 것을 막는다) · 이어받는 기준은 env 이름.
  **`outputs` 의 등급표·토큰 통로·라우트표 세 줄은 WS-작성으로 넘겼다**(2026-09-26 사용자 — 통로 없이 표만 넣으면 gate.test 유령 줄 검사가 빨강). `/auth/me` 에 `hasLogin` 은 **넣지 않는다**(사용자 — 계정 없는 줄은 서버가 `BAD_ENV` 로 거절한다).
  **남은 것 없음.** 명세 [도메인/인증.md](spec/도메인/인증.md) §7 「envs[] 한 줄」 · §8.8
- **WS-작성 ① 서버 통로 — 반영 완료 (2026-09-26, PR #79).** 사용자가 셋(서버 → 화면 WS-E ② → 에이전트 ③)으로 나눴다.
...
4. Grafana 프로비저닝 — Postgres 데이터소스 + 대시보드 JSON
   패널: 성공률 추이 / 평균 소요시간 / 실패 TOP10 케이스 / 최근 실행 목록 / 가장 오래된 미확정 / 작성에 걸린 시간
   (뒤의 둘은 2026-09-26 — 칸 단위 권한이 필요하다. 명세 데이터모델 「대시보드 칸 권한」)
...
> **Grafana 실행 패널(처음 넷)은 새 컬럼에 기대지 않으므로 먼저 해도 된다.**

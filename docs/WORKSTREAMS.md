...
**2026-09-22 에 아래 소유 경로 표로 `WS-작성` 이 들어갔다** — 서버·명세·맥 스크립트가 거기 담겼다.
**화면은 여전히 WS-E 소유 경로 안이고 그 갈래가 가져가지 않는다.**
...
**위의 「테스트 작성」 화면과 같은 덩어리다** — 화면·서버·명세를 한꺼번에 건드려
...
> ④ `DESIGN.md` 모달 규칙 일곱 중 **넷**은 아무 검사도 안 본다 (덮개·애니메이션·색).
...
> ① 드롭다운 비밀값 · ② 화면 그물 · ③ `aria-describedby` · ④ 모달 규칙 **넷** ·

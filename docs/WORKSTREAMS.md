...
>
> **여섯 중 이번 PR(「마무리」, 2026-09-20)이 닫은 것은 하나도 없다.** 그 PR 은 명세·증적 문서·설정값을
> 건드렸고 화면 파일은 `layout.ts` 와 `runState.ts` 둘뿐이라 ①③④ 는 손도 안 댔다.
> **② 화면 그물 · ⑤ 비밀값 데모 케이스 · ⑥ `@fastify/static` 취약점도 그대로 남는다** —
> 여섯 전부 여전히 열려 있다. 그물이 지금 몇 대 몇인지는 위 두 명령이 센다.
>
...
>
> **⑧ 증적 문서 형식을 고르는 자리가 화면에 없다** (2026-09-20 검사에서 찾았다).
> SPEC §8.4 의 「내는 형식」 표가 **「결과 화면에서 골라 만든다」**고 정본으로 적어 뒀는데
> `apps/admin/src/web/RunResult.tsx` 가 `api.makeEvidence(runId, 'PDF')` 로 **PDF 를 박아 놨다.**
> 엑셀·HTML 을 고르는 UI 가 화면 어디에도 없다.
> **완료 기준 상자 넷이 여기에 매달려 있다** — 형식 골라 만들기 · 엑셀 `받기` ·
> 엑셀 표 모양 · 엑셀 두 번 뽑기. **지금은 누를 버튼이 없어서 못 채운다.**
> 서버·명세·엑셀 만들기는 전부 준비돼 있다 — **화면 한 자리만 비었다.**
> 버튼 글자는 §8.4 가 이미 정해 뒀다(PDF·HTML 은 `열기 ↗`, 엑셀은 `받기`).

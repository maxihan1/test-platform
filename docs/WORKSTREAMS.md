...
**그 PR 은 명세만 세웠다.** 계약 블록은 전부 `상태: 대기` 였다 — 타입·DB 칸 둘은 PR #74, 집계는 PR #76, 설정 `envs[]` 는 PR #77 에서 `반영 완료` 가 됐고 나머지(작성 통로 · `outputs` 권한 세 줄)는 아직 `대기` 다.
...
- **WS-작성** — 서버 통로(`compare` · `env` · `startUrl` · 화면만의 `submit` 예외 · 집기 응답 `target` · `outputs` · `finish` 의 `diffs`) ·
  **`outputs` 통로를 만들 때 등급표(`'POST /api/authoring/requests/:id/outputs': 'operator'`) · 토큰 통로 · 라우트표(`{ 종류: '작성요청', 칸: 'id' }`) 세 줄을 같은 커밋에 넣는다** — WS-F 에서 넘어왔다(인증 §7 두 번째 계약 블록, 상태 대기) ·
